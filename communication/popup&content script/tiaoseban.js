	...
	g.prototype = {
		...
		createHtml: function () {
			...
			c.each(a.lately, function (a, b) {
				...
				d +=
					...
					b +
					');" class\x3d"paigusu-lately" data-color\x3d"' +
					b +
			...
			a = [
				...
				'\x3cdiv style\x3d"width:16px;height:16px;border-radius: 8px;background: rgba(' +
					a.color.r +
					"," +
					a.color.g +
					"," +
				...
				'\x3cdiv style\x3d"width:10px;height: 10px;background-color: #fff;border-radius: 5px;box-shadow: 0 0 2px #ccc;position: absolute;left:' +
					(a.colorSlider - 5) +
					'px; z-index:1;" id\x3d"paigusu-slider-btn"\x3e\x3c/div\x3e',
				...
				'\x3cp style\x3d"color:#666;text-indent: 10px;margin:0;"\x3e' +
					a.title +
					"\x3c/p\x3e",
		...
		initDefaultColor: function () {
			...
			if (1 == a.length) {
				...
				3 == a.length
					? ((b = a[0]), (d = a[1]), (e = a[2]), (a = 1))
					: 4 == a.length
					? ((b = a[0]), (d = a[1]), (e = a[2]), (a = a[3]))
					: ((b = 42), (d = 0), (e = 255), (a = 1)),
					(b = {
						...
						a: parseFloat(
							0.9 < parseFloat(a).toFixed(1) ? 1 : parseFloat(a).toFixed(1)
						),
					}),
					(d = this.rgbToHex(b)),
					(e = this.rgbToHsb(b));
			...
			this.set({
				...
				hsb: e,
				bg: a,
				ident: {top: 150 - (e.b / 100) * 150 - 6, left: (e.s / 100) * 230 - 6},
		...
		latelyColor: function () {
			if (this.options.recommend) {
				...
				b
					? (b = b.split("-"))
					: ((b =
							"0,0,51,1 0,51,102,1 0,102,153,1 0,153,204,1 0,204,255,1 102,0,51,1 102,51,102,1 102,102,153,1 102,204,204,1 102,255,255,1".split(
								" "
							)),
		...
		getPosition: function () {
			...
			b - e.right > a.width
				? this.set({positionLeft: e.right + 5})
				: this.set({positionLeft: e.left - a.width - 5});
			d - e.bottom > a.height
				? this.set({positionTop: e.top})
				: this.set({positionTop: e.bottom - a.height});
		},
		doploy: function () {
			var a = this.options;
			c("#paigusu-slider-btn").css("left", a.colorSlider - 5);
			var b = this.hsbToHex({h: (a.colorSlider / 160) * 360, s: 100, b: 100}),
				d = this.hexToRgb(b),
				e = {h: (a.colorSlider / 160) * 360, s: 100, s: a.hsb.s, b: a.hsb.b},
				b = this.hsbToHex(e),
				f = this.hexToRgb(b);
			f.a = a.color.a;
		...
		inputSetColor: function () {
			var a = this.options.hsb,
				b = this.hsbToRgb({h: 0 == a.h ? 250 : a.h, s: 100, b: 100}),
				d = 150 - (a.b / 100) * 150 - 6,
				e = (a.s / 100) * 230 - 6,
			...
			c("#paigusu-alpha-block").css(
				"background",
				"linear-gradient(to right, rgba(0,17,255, 0) 0%, rgba(" +
					b.r +
					"," +
					b.g +
					"," +
		...
		on: function () {
			...
			c("#paigusu-tab")
				.off()
				.on("click", function () {
					c("#paigusu-rgba").is(":hidden")
						? (c("#paigusu-rgba").show(), c("#paigusu-rex").hide())
						: (c("#paigusu-rgba").hide(), c("#paigusu-rex").show());
				});
			c("#paigusu-palette")
				.off()
			...
			c("#paigusu-slider")
				...
				.on("mousemove", function (b) {
					b = b || f.event;
					f.paigususlider &&
						(a.set({colorSlider: 0 > b.offsetX ? 0 : b.offsetX}), a.doploy());
				})
				.on("mouseup", function () {
					f.paigususlider = !1;
				})
			...
			c("#paigusu-alpha")
				...
				.on("mousemove", function (b) {
					...
					f.paigusualpha &&
						(c("#paigusu-alpha-btn").css("left", b.offsetX - 6),
						(a.options.color.a = parseFloat(
							((0 > b.offsetX ? 0 : b.offsetX) / 160).toFixed(1)
						)),
						a.setInputColor());
				})
				.on("mouseup", function () {
			...
			c(
				'input[name\x3d"paigusu-r"],input[name\x3d"paigusu-g"],input[name\x3d"paigusu-b"],input[name\x3d"paigusu-a"]'
			).on("keyup", function () {
			...
			c('input[name\x3d"paigusu-hex"]').on("keyup", function () {
				...
				if (3 == b.length || 6 == b.length)
					(b = a.hexToRgb(b)),
						(b.a = 1),
						a.set({color: b, hex: a.rgbToHex(b), hsb: a.rgbToHsb(b)}),
			...
			c(".paigusu-lately").on("click", function () {
				var b = c(this).data("color"),
					d = b.split(","),
					e = {r: parseInt(d[0]), g: parseInt(d[1]), b: parseInt(d[2])},
					b = e;
				b.a = d["3"] || 1;
				d = a.rgbToHex(e);
				a.callBack(a.eve, {
